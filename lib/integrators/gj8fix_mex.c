/* Name:     gj8fix_mex()
Purpose:     Fixed-step 8th-order Gauss-Jackson initial value ODE solver.
             Converts MATLAB inputs into C inputs for the gj8fix.c
             integrator.  Calls the gj8fix() integrator and returns
             outputs to MATLAB.
             TurboProp Version 4.0.

Call:        [T,Y] = gj8fix_mex(odefun,tspan,y0,options,extras)
...
Inputs:      odefun  = string with the name of the derivative function 
             ...
             options = Array of two doubles.
                       ...
                       that the time step evenly divides the time span.
                       If your time step evenly divides the time span, 
                       but numerical issues make it so that it appears it
                       does not, use a larger tolerance.  A good value 
                       is normally 1e-6.
...
#define SOLVER gj8fix
#define SOLVERSTR "gj8fix"
